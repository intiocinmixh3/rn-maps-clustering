...
const ClusteredMapView = forwardRef(({ clusteringEnabled = true, spiralEnabled = true, spiderfyOnMaxZoom = true, animationEnabled = true, radius = 40, maxZoom = 20, minZoom = 1, minPoints = 2, extent = 512, nodeSize = 64, clusterColor = '#00B386', clusterTextColor = '#FFFFFF', clusterFontFamily, spiderLineColor = '#FF0000', edgePadding = { top: 50, left: 50, right: 50, bottom: 50 }, children, onRegionChangeComplete, onClusterPress, renderCluster, tracksViewChanges, ...restProps }, ref) => {
    ...
    return (<MapView {...restProps} ref={mapRef} onRegionChangeComplete={handleRegionChange}>
        {markers.map((marker) => {
            if ('cluster' in marker.properties) {
                const cluster = marker;
                const onPress = () => handleClusterPress(cluster);
                return renderCluster ? (renderCluster(cluster, onPress)) : (<ClusteredMarker key={`cluster-${cluster.id}`} {...cluster} onPress={onPress} clusterColor={clusterColor} clusterTextColor={clusterTextColor} clusterFontFamily={clusterFontFamily} tracksViewChanges={tracksViewChanges}/>);
            }
            const point = marker;
        ...
        {spiderMarkers.map((marker) => {
            ...
            return (<React.Fragment key={`spider-${marker.index}`}>
              <Polyline coordinates={[
                    marker.centerPoint,
                    { latitude: marker.latitude, longitude: marker.longitude },
                ]} strokeColor={spiderLineColor} strokeWidth={1}/>
